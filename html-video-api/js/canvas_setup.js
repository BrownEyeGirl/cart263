...
window.onload = function () {
    // get the canvas
    let canvas = document.getElementById("testCanvas"); // each canvas has its own individual context
    //get the context
    ...
    // draw a rect
    context.fillRect(canvas.width / 2, canvas.height / 2, rectSize, rectSize); // pixel graphics on a canvas 
    // cut out a rect inside. cuts a shape out of the canvas
    context.clearRect(canvas.width / 2 + rectSize/4, canvas.height / 2 + rectSize/4, rectSize/2, rectSize/2);


    // styling (note: must be done after the creation of the shapes)
    ...
    /* 
        Notes 
            ...
            moveTo(x, y) -> A useful function, which doesn't actually draw anything but becomes part of the path 
            list described above. You can probably best think of this as lifting a pen or pencil from one spot on a 
            piece of paper and placing it on the next. -> very useful if you want to draw more complex shapes.
    */

    context.beginPath(); // open shape 
    context.arc(xPos,yPos,radius,startAngle,endAngle, true);
    context.fill(); // set the fill
    context.lineWidth=2; //change stroke
    context.stroke();//set the stroke
    context.closePath(); // close shape 
